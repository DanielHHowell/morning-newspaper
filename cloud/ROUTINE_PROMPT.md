...
6. Deliver: if the environment variables SMTP_USER, SMTP_PASS and PRINTER_EMAIL are all set, run `python3 bin/send_email.py editions/TODAY.pdf` (it emails the PDF to the printer via Epson Connect). If any is missing, skip delivery and say so. Never print or echo those values.
