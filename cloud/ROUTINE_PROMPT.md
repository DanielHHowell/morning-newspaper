...
5. PDF: `MAX_PAGES=10 python3 bin/build.py TODAY` -> `editions/TODAY.pdf`. Confirm the file exists and note the page count it prints.
6. Deliver to the printer (Epson Connect email print): `SMTP_HOST=... SMTP_USER=... SMTP_PASS='...' PRINTER_EMAIL=... python3 bin/send_email.py editions/TODAY.pdf` (the live routine carries the real values; they are not in this repo). If the connection is refused or times out, report "SMTP blocked by network policy".
